...
class JDJellyButton
{
    ...
    init() {
       Container = JelllyContainer(frame: CGRect(x: 0, y: 50, width: 400, height: 400))
    }
    ...
    {
        print ("[JDJellyButton] attachtoView")
        RootView = rootView
...
{
    // Main Button 被點中，要更新自己的位置
    func MainButtonHasBeenTap(touch:UITouch)
    {
        //print ("[JDJellyButton:MainButtonDelegate] MainButtonHasBeenTap")
    ...
    func JDMainButtonHasBeenTap() {
        print ("[JDJellyButton:MainButtonDelegate] JDMainButtonHasBeenTap")
        delegate?.JDMainButtonHasBeenTap()
    }
